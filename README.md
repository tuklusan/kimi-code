...
- **`WaitFor` tool wait cap** — the fork evaluated raising this beyond the upstream 10-minute default so long-running verification / QA / review runs could sit on a single wait, but every constant we surfaced (schema `.max()`, `.describe()`) is inlined into the tool's JSON that becomes part of the LLM system prompt, and any bump shifts the compaction prompt tokenization by ~1 token — enough to break several upstream token-count-sensitive tests. The fork keeps the upstream 10-minute cap for that reason. The tool's own description already tells callers "A timeout is not an error: the tool returns the tasks that are still running, and you can call it again to keep waiting" — so long waits are just a poll loop with 10-minute strides.
