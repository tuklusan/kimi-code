...
**➡ [Download the latest fork release](https://github.com/tuklusan/kimi-code/releases/latest)** — native binaries for linux-x64 / linux-arm64 / darwin-x64 / darwin-arm64 / win32-x64 / win32-arm64, tagged `kimi-code-sanyalnet-cli-vX.Y.Z`. Current recommended baseline is **rev 1.0.3** (Nemotron 3 Ultra 550B A55B default, filesystem-driven initial-prompt autoload, full six-role SDLC company, HTTP 400 & 404 treated as retryable, telemetry suppressed by default).
...
- **Telemetry suppressed by default** — no events are sent to the upstream telemetry endpoints (`telemetry-logs.kimi.com` / `telemetry-logs.kimi.ai`). All three initialization paths (CLI TUI, `kimi web` host, and the headless print/`kap-server` host) now require an explicit opt-in, so the default build never contacts a telemetry host. Opt back in with `KIMI_ENABLE_TELEMETRY=1`; the upstream `KIMI_DISABLE_TELEMETRY=1` and config `telemetry = false` still force it off and always win.
