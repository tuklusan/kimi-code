...
- **`WaitFor` tool wait cap raised to 7 days** — upstream capped the `WaitFor` background-task waiter at the bash background default (10 min), which made long-running verification / QA / review runs drop out of their wait early and forced the caller into a poll loop. The fork sets the cap to a practically unlimited 7 days so a multi-hour SDLC run can just wait.
