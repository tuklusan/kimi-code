...
- **`WaitFor` tool wait cap** — the fork evaluated raising this beyond the upstream 10-minute (600 s) default so long-running verification / QA / review runs could sit on a single wait. Two blockers, empirically proven by bisect (see commits `be9dc8dc`, `2b0a66d2`, `1d3b7627`): (1) both surfaces where the constant appears — the zod `.max()` and the `.describe()` text — get emitted into the tool's JSON that lives in the LLM system prompt, so any bump shifts the compaction prompt tokenization by ~1 token and breaks several upstream token-count-sensitive tests in `fullCompaction.test.ts`; (2) two upstream inline snapshots (`test/tool/tool.test.ts` and `test/agent/loop/loop.test.ts`) embed the tools JSON verbatim including a SHA-256 hash of the whole tool schema, so any value change fails snapshot equality even if tokenization matches. Raising the cap safely would require regenerating those snapshots via `vitest -u` (which we can only do in CI). The fork keeps the upstream 600 s cap for that reason. The tool's own description already tells callers "A timeout is not an error: the tool returns the tasks that are still running, and you can call it again to keep waiting" — so long waits are just a poll loop with 10-minute strides.
