...
**➡ [下载最新分支发行版](https://github.com/tuklusan/kimi-code/releases/latest)** —— 覆盖 linux-x64 / linux-arm64 / darwin-x64 / darwin-arm64 / win32-x64 / win32-arm64 的原生二进制，标签为 `kimi-code-sanyalnet-cli-vX.Y.Z`。当前推荐基线为 **rev 1.0.3**（Nemotron 3 Ultra 550B A55B 作为默认模型，文件系统驱动的初始提示自动加载，完整的六角色 SDLC 软件公司，HTTP 400 与 404 视为可重试，遥测默认关闭）。
...
- **遥测默认关闭** —— 不再向上游遥测端点（`telemetry-logs.kimi.com` / `telemetry-logs.kimi.ai`）发送任何事件。三条初始化路径（CLI TUI、`kimi web` 主机，以及无头 print/`kap-server` 主机）都改为需要显式开启，因此默认构建不会联系任何遥测主机。用 `KIMI_ENABLE_TELEMETRY=1` 重新开启；上游的 `KIMI_DISABLE_TELEMETRY=1` 与配置 `telemetry = false` 仍然强制关闭并始终优先。
...
- **模型 400 视为可重试** —— 严格的 OpenAI 兼容网关（尤其是 NVIDIA NIM）在预热/自动扩缩期间可能短暂返回 HTTP 400，因此本分支把一般的 400 归为与 429 同类的可重试。确定性的 400 子类仍然快速失败，不浪费重试预算：上下文超长（context-overflow）与请求过大（request-too-large）保留各自的裁剪/重发恢复流程，图像格式错误保留去除媒体后重发的流程。
- **`WaitFor` 工具等待上限提升到 999 秒（约 16 分 40 秒）** —— 比上游默认的 10 分钟（600 秒）多出 66% 的余量，让长时间的验证 / QA / 评审任务能在一次等待中完成。这次提升并不简单：常量出现在工具的 zod `.max()` 与 `.describe()` 文本中，两者都会被写入进入 LLM 系统提示的工具 JSON，而两个上游内联快照（`test/tool/tool.test.ts`、`test/agent/loop/loop.test.ts`）逐字固定该 JSON，包括整个工具 schema 的 SHA-256 哈希。选 `999` 是因为它与 `600` 保持相同的 3 字符长度（因此提示 tokenization 不变 —— 4 位数值会偏移一个 token 并破坏 `fullCompaction.test.ts` 的 token 数断言），两个快照通过 CI 上的 [`regen-snapshots`](.github/workflows/regen-snapshots.yml) 工作流（`pnpm exec vitest run --update`）重新生成，因为 schema 哈希只能通过运行测试套件重新计算。对于超过 999 秒的等待，工具自身的契约依然适用："超时不是错误：该工具返回仍在运行的任务列表，你可以再次调用以继续等待。"
